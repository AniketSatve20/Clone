    ...
    <div className="min-h-screen bg-gradient-crypto flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-blockchain-card border border-blockchain-border rounded-xl p-8 space-y-6 shadow-lg backdrop-blur-xl">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="text-sm text-neon-cyan hover:text-neon-purple font-medium flex items-center gap-2"
                      >
                        {wallet.chainName}
                      ...
                      {showChainDropdown && (
                        <div className="absolute right-0 mt-2 w-48 bg-blockchain-card border border-blockchain-border rounded-lg shadow-lg z-50">
                          {chains.map((chain) => (
                            <button
                              key={chain.id}
                              className="w-full text-left px-4 py-2 hover:bg-blockchain-hover text-sm text-gray-300 border-b border-blockchain-border last:border-b-0 transition-colors"
                              onClick={() => {
                                // switchNetwork(chain.id);
                                setShowChainDropdown(false);
                              }}
                            >
                              <span className="font-mono">{chain.icon}</span> {chain.name}
                            </button>
                          ))}
                        </div>
                      )}
          ...
          {error && !['email', 'verification', 'wallet', 'connected'].includes(step) && (
            <div className="bg-danger/10 border border-danger/50 rounded-lg p-3 text-sm text-danger">
              {error}
            </div>
          )}
        </div>
